...
const G6_ACCOUNTS = `select distinct a.poid_id0 from account_t a join service_t s on s.account_obj_id0 = a.poid_id0 where s.poid_type = '${SERVICE_TYPE}'`;
...
export type Group6Usage = {
  ...
  derived: {
    productCount: number;
    modelCount: number;
    tierRange: { min: string; max: string };
  ...
  tiers: Array<{ tier: string; events: number; m30: number; m35: number }>;
...
export async function getGroup6Usage(range: UsageRange = {}): Promise<Group6Usage> {
  ...
  const empty: Group6Usage = {
    ...
    derived: {
      productCount: 0, modelCount: 0, tierRange: { min: "", max: "" }, medianDailyPrompts: 0,
    ...
    tiers: [], scatter: [], productMix: [], peakHours: [],
  ...
  const headlineRows = await rows(
    `select 'window_min' k, to_char(date '1970-01-01' + min(e.start_t)/86400,'YYYY-MM-DD HH24:MI') v from ${USAGE_FROM} ${W}
     ...
     union all select 'usage_revenue', to_char(round((select sum(bi.amount) from event_bal_impacts_t bi join event_t e2 on e2.poid_id0 = bi.obj_id0 where e2.poid_type = '/event/session/usagegr6' and bi.resource_id = 840),2)) from dual
  ...
  // Usage-intensity bucket (light->heavy) parsed from transaction id, split by model.
  // L01-L10 are generated usage-volume markers, not plan or price tiers.
  const tierRows = await rows(
    `select regexp_substr(u.transaction_id2_g6,'_L([0-9]+)_',1,1,null,1) tier, u.model_code2_g6 model, count(*) nn
       from ${USAGE_FROM} ${W} group by regexp_substr(u.transaction_id2_g6,'_L([0-9]+)_',1,1,null,1), u.model_code2_g6`,
  );
  ...
  const acctBillRows = await rows(
    `select account_obj_id0 acct, round(sum(total_due),2) due from bill_t where account_obj_id0 in (${G6_ACCOUNTS}) group by account_obj_id0`,
    1000,
  );
  ...
  for (const r of acctBillRows) acctDue[n(r.ACCT)] = n(r.DUE);
  ...
  const tierMap = new Map<string, { tier: string; events: number; m30: number; m35: number }>();
  for (const r of tierRows) {
    const tier = s(r.TIER);
    if (!tier) continue;
    const cur = tierMap.get(tier) ?? { tier, events: 0, m30: 0, m35: 0 };
    const v = n(r.NN);
    cur.events += v;
    if (s(r.MODEL) === "3.5") cur.m35 += v;
    else cur.m30 += v;
    tierMap.set(tier, cur);
  }
  const tiers = [...tierMap.values()].sort((a, b) => a.tier.localeCompare(b.tier));
  ...
  const tierLabels = tiers.map((t) => t.tier).sort();
  const tierRange = { min: tierLabels[0] ?? "", max: tierLabels[tierLabels.length - 1] ?? "" };
  ...
  const rangeRows = await rows(
    `select to_char(date '1970-01-01' + min(e.start_t)/86400,'YYYY-MM-DD') mn,
       ...
       from event_t e where e.poid_type = '/event/session/usagegr6'`, 2);
  ...
  const subRows = await rows(
    `select count(*) total, sum(case when pp.status = 1 then 1 else 0 end) active
      ...
      where s.poid_type = '${SERVICE_TYPE}'`, 2);
  ...
  const acctPlanRows = await rows(
    `select pp.account_obj_id0 acct, min(pl.name) plan
      ...
      where s.poid_type = '${SERVICE_TYPE}' group by pp.account_obj_id0`, 1000);
  ...
  const acctStateRows = await rows(
    `select obj_id0 acct, nvl(state,'—') state from account_nameinfo_t where obj_id0 in (${G6_ACCOUNTS})`, 1000);
  const acctState: Record<number, string> = {};
  for (const r of acctStateRows) acctState[n(r.ACCT)] = s(r.STATE, "—");
  ...
  const result: Group6Usage = {
    ...
    pricing,
    notes: [
      `Scope: all ${totalUsers} Group 6 accounts on /service/nextaig6 (L${tierRange.min} → L${tierRange.max} usage-intensity buckets, not plan tiers).`,
